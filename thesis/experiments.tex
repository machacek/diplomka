...
\begin{table}
  ...
  \caption[Correlations of the segment-level and the sentence-level
...
short segments.  As you saw in the previous section, evaluating the annotated
...
sequence $s$. If more segments in the database have the same minimal distance to the
``unseen'' segment, we compute the average of their ranks.
...
point will be the good translation. You can see an illustration of this
situation in two dimensions in Figure \ref{translation-space-illustration}. If
we do not have enough candidates in the database it is quite likely that for a
given unseen candidate the closest candidate is the good translation and not
any other candidate translation of similar quality.
...
\begin{figure}
    ...
    \caption[Counts of matched segments with respect to the edit distance]
...
\caption[Comparisons of the unseen and the closest segments]{Comparisons of the unseen
and the closest segments' ranks. This table shows how often the rank of the closest segment in the
database was worse, equal or better than the original rank of the ``unseen''
...
\begin{figure}
    ...
    \caption[Comparisons of the unseen and the closest segments]{Comparisons of
      the unseen and the closest segments' ranks with respect to the edit
  distance. The results in this figure are computed on all of the systems.}
...
You can also see how these relative frequencies vary with the change of the edit
...
\subsection{Enhancing Reference Translation}
...
number of possible correct translations is very high \parcite{bojar2013scratching}. It
...
\begin{table}
  ...
  \caption[Overall ranking according to \metric{SegRanksBLEU} and
...
segments are really good in terms of the absolute quality. We have to assume
...
\metric{SegRanksBLEU} is even a little bit lower).
...
chooses the best translation in the equation \ref{eq:sentence-translation}, we
get the following.
...
The parallel data used to train the phrase tables consist of 14.83 million
...
and new metrics are created by subclassing the base \textit{Scorer} class. Since
...
\textit{PythonScorer} which is a universal wrapper around an arbitrary Python
...
sentences which were translated differently\footnote{58.6 \% of all test set
    ...
    \metric{SegRanksBLEU}, 62.4 \% by \metric{PenalizeUnknown} and 83.4 \% by
\metric{ExactMatch}} to the baseline and by the evaluated system. Then we
...
tuned solely to \metric{BLEU} in the automatic evaluation. This was expected,
since the best performing system according to \metric{BLEU} should be the one
which was tuned by \metric{BLEU}. However, you can see that the system tuned by
\metric{PenalizeUnknown} has both the \metric{BLEU} and \metric{CDER} scores
...
tuned to \metric{PenalizeUnknown}.  This means that forcing the system to
...
We should say here, that this experiment should be performed more precisely to
obtain reliable results. The problem with MERT is that it is not
deterministic in two aspects: First, MERT method can converge to only a local
optimum, this depends on the point it started. Second, the used implementation of
...
different weights. The fact that system tuned to \metric{PenalizeUnknown} was
...
takes many more iterations for \metric{ExactMatch} to get to the value of 0.5 that
it takes for \metric{PenalizeUnknown} to get to the value of 0.7. 
