...
Machine translation systems often translate a short source segment
identically. This was on of our main motivation for ranking translations of
short segments. As you saw in the previous chapter, evaluating the annotated
systems using the short segments annotations works even it has some
shortcomings.  It would be very usefu if we could use the database of
annotations to evaluate also yet unseen systems. The more annotated systems we
have in the database the more likely is that an unseen MT system produce a
translation of short segment which is already in the database. We will call
this situation a \metoda{hit}, if the translated segment is not already
annotated, we will call it a \metoda{miss}.
...
\metoda{Ratio of wins (ignoring ties)} only on the hits. Because we didn't have
any spare systems which were not annotated, we do the following trick: in each
step we choose one system and removed its segments from the database of
annotations, then we try to match the system's segments with the segments in
the database and compute the \metoda{Ratio of wins (ignoring ties)} only on
that segments. We call this trick \metoda{leave-one-out}.
...
\noindent
Both of the hit segments are translated very well and we can say that they are
also very easy to translate. On the other hand, the missed segments
understandable but not perfect. Compared to the green segment, they are also
