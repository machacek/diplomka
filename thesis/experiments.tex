...
two dimensions in the figure \ref{translation-space-illustration}. If we do not
have enough candidates in the database it is quite likely that for a given
unseen candidate the closest candidate is the good translation and not any
other candidate translation of similar quality.
...
To support the above statements we have performed an analysis on a sample of
the data. In the table \ref{segments-closest}, we listed a few candidate
segments together with the corresponding closest segments from the database and
their distances. We also report whether the closest segment was ranked better
than the ``unseen'' one.
