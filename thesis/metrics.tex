...
parameters at all (\metric{UPC-STOUT} and \metric{UPC-IPA}), tune them manually
(\metric{AMBER}, \metric{VERT*}) or use generic optimization techniques like
hill climbing which does not have to find the optimum (\metric{METEOR}).
...
\perscite{wmt14-metric-upc} combine various metrics implemented in the
\textsc{Asiya} toolkit.\footnote{\url{http://nlp.lsi.upc.edu/asiya/}}. The set
of the metrics they use contains 6 lexical based metrics (\metric{WER},
\metric{PER}, \metric{TER}, \metric{METEOR-exact}, etc.), 2 shallow parsing
...
The first variant, \metric{UPC-STOUT}, is defined as the uniformly weighted
linear interpolation of the best performing metrics (measured on previous years
metrics data). The second variant, \metric{UPC-IPA} is a light version of
\metric{UPC-STOUT}.  It considers only the metrics which require no or little
language resources.
...
the lexical based metrics (\metric{NIST}, \metric{TER}, \metric{WER},
\metric{PER}) perform very poorly. However, \metric{DiscoTK-light} which
...
\metric{DiscoTK-party-tuned} and \metric{DiscoTK-party}.
...
\subsubsection{Out-of-English Results Analysis}
...
(\metric{DiscoTK-party-tuned}, \metric{BEER}, \metric{RED*} and
\metric{Meteor}) are significantly better better than metrics which do not
(\metric{UPC-*}, \metric{VERTa-*}). An interesting contrast is between the
\metric{DiscoTK-party-tuned} and \metric{DiscoTK-party} metrics, which differ
only in the tuning. It shows that the tuning is very important. Comparing \metric{DiscoTK-party}
to \metric{DiscoTK-light} shows that discourse tree based metrics are not very useful in MT evaluation.
