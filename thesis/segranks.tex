...
which we create a database of human annotations which could be reused
...
in the following two chapters. In this chapter, we describe the way of
...
disadvantages of this method is that sentences are quite long and therefore
...
better or worse than another sentence and therefore it is more difficult for
...
nodes which cover source segments of a given maximum length. This is exactly
described in Algorithm \ref{segment:extraction}. Finally, we project these
...
\begin{figure}
    ...
    \caption[An illustration of candidate segments extraction process]{ An
      ...
      not all words are always covered by extracted segments.
...
\begin{itemize}
  ...
  \item This is very similar to the previous one. A system could translate
    ...
    tree and extract also the longer segments which consist of already
    extracted shorter segments. However, if we use this approach, the amount of
    annotation work would multiply several times. Furthermore, the longer segments
    are more difficult to rank and the chance that systems' candidates will be
    identical (so that we can merge them for annotation) is lower.

  \item Extracted short segments do not cover the whole sentence. For example
    in Figure \label{tree-align}, the words `jsou' and `pak' are not part of any
    extracted segment. We would avoid this problem if we set the variable
    \pojem{minLength} to zero but it would generate a high number of short
    segments to annotate. \XXX{Spocitat kolik, mozna ale v dalsi subsekci,
    mozna tam udelat i graf}
  ...
  \item Some segment candidates are much more important to convey the meaning
    ...
    ranked segments and system B better than system A in the third segment, it
    ...
    important for the quality of translation than the first two. We are afraid
    that it is not possible to easily avoid this problem. However, we also
    believe that this problem is not so serious and that possible differences
    in the importance of individual segments cancel out.
...
section, we describe what data we have used and how we prepared them for the
...
also helped to reduce the number of extracted segments to be annotated. The
...
Please place unacceptable candidates to the position ``Garbage''
...
dragging and dropping them to appropriate rank positions. When all candidates of all
...
\hline
   & 2773 & 8333 & 6 days, 14:22:57 & 23895 & 68 & 0.593 & 0.397 \\
    ...
    CZK, average time in seconds spent on one segment annotation and
...
knowing anything about the task.  Values of $\kappa$ can be therefore compared in principle
across various annotation experiments. The maximum value is 1 which means
that annotators always agree.  The zero value means that annotators agree as
often as they would by chance.
...
in the table \ref{agreements} together with the corresponding $\kappa$ values
...
than they are in WMT14. However, they are still quite low and we expected them
